...
def main():
    ...
    try:
        ...
        corpusFile='/Users/zhangyiyang/Desktop/毕业论文相关/2020-8-20_zh.tsv'
        goalFile='/Users/zhangyiyang/Desktop/毕业论文相关/2020-8-20_zh_extraction.csv'

        # print(client.get(suffix='statuses/user_timeline.json'))
        ID=client.get(suffix='statuses/queryid.json',params={'mid':'JgQ35lm3m','type':1,'isBase62':1})['id']
        print(ID)
        with open(corpusFile, 'r') as tsvfile:
            lineReader = csv.reader(tsvfile, delimiter='\t', quotechar="\"")
            for row in lineReader:
                print(row)
                mid=re.search(r'.*/(.*)',row[1]).group(1)
                print(mid)
                if mid:
                    print(ID)
                    output=client.get(suffix='statuses/show.json',params={'id':ID})
